...
(function($env){
    ...
    // Override this to supply an alternative URL implementation
    // Instantiated with a path
    // Fields: domain, hash, href
    $env.URL          = java.net.URL;
    ...
    $env.connection = function(xhr, responseHandler){
      if($env.mockConnection) {
        var response = $env.mockConnection.request(xhr);
        xhr.status = response.status();
        xhr.readyState = 4;
        xhr.responseText = response.body().join();
      } else {      
