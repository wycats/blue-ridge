...
Matcher.create("Be", {
	...
	"==": function() {
		return this.__equals(this.actual, this.expected);
	},
	"<": function() {
	...
	__equals: function(actual, expected) {
	  return Object.isArray(expected) && Object.isArray(actual)
			? expected.size() == actual.size() &&
					expected.all(function(element, index) { return this.__equals(element, actual[index]) }.bind(this))
			: actual == expected;
	}
});
Matcher.addHelpers({
...
Matcher.addHelpers({
	beInstanceOf: function(constructor) {
		return new Matcher.InstanceOf(constructor);
	}
});

Matcher.create("InstanceOf", {
	initialize: function(constructor) {
		this._constructor = constructor;
	},
	matches: function(actual) {
		this.actual = actual;
		return this.actual instanceof this._constructor;
	},
	failureMessage: function() {
		return "expected target to be an instance of " + this._constructor;
	},
	negativeFailureMessage: function() {
		return "expected target to not be an instance of " + this._constructor;
	}
});
Matcher.addHelpers({
...
Context = Class.create({
	...
	flunk: function(message) {
		throw new Expectation.Unmet("Flunked: " + message);
	}
});
